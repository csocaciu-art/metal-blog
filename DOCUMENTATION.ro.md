...
- **Ce este:** Un instrument de formatare a codului care impune automat un stil consistent (cum ar fi indentarea și spațierea) în întregul proiect.
- **De ce este folosit:** Asigură că codul arată curat și profesional, indiferent de cine îl scrie. Folosim comanda `npm run format` pentru a curăța codul.

### Vitest

- **Ce este:** Un framework de teste unitare foarte rapid, propulsat de Vite.
- **De ce este folosit:** Ne permite să scriem "teste" care verifică automat dacă codul nostru funcționează așa cum ne așteptăm. De exemplu, verificăm dacă logica noastră de încărcare a datelor postărilor gestionează corect fișierele lipsă. Rulăm testele cu `npm test`.
