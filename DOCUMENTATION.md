...
- **What it is:** A code formatter that automatically enforces a consistent style (like indentation and spacing) across the entire project.
- **Why it's used:** It ensures that the code looks clean and professional, regardless of who writes it. We use the command `npm run format` to tidy up the code.

### Vitest

- **What it is:** A blazing fast unit test framework powered by Vite.
- **Why it's used:** It allows us to write "tests" that automatically verify our code works as expected. For example, we check that our blog post data loading logic handles missing files correctly. We run tests with `npm test`.
